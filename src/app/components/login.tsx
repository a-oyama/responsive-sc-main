...
const Login = () => {
  ...
  const [loading, setLoading] = useState(false) // ローディング変数
  const [message, setMessage] = useState('')    // メッセージ変数

  // フォームの設定,useFromでフォーム状態を管理
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit: SubmitHandler<Schema> = async (data) => {
    ...
    try {
      // ログインはsupabase.auth.signInWithPasswordを使用
